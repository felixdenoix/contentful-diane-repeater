...
class App extends React.Component {
  ...
  componentDidMount() {
    ...
    if (this.state.value) {
      ...
      this.props.sdk.space
        .getAsset(this.state.value[0].sys.id)
        .then(asset => this.setState(state=>({ assets: [...state.assets, asset] })))
  ...
  */
  ...
  createNewAssetFromFiles = async files => {
    ...
    // this.setState((state) => ({ content: [...state.content, imageFile] }));
  ...
  */
  ...
  renderImageElement() {
    ...
    } else if (!this.state.isDraggingOver && this.state.assets) {
